...
void Main()
{
	var processor = new Processor( @"..\app",@"User",@"UserCreateUpdate");
	processor.Scaffold();
}
class Processor
{
		...
		bool copyScaffold = false)
	{
		...
		_copyScaffold = copyScaffold;
		 ...
		 _fileTypes = new[] { @".xml" , ".kt" };
		 _exclude = new[] { @"bower_components", "build\\" };
	}
	...
	public string ReplaceAll(string text)
	{
		...
		var replaced = text
			.Replace(ToSnakeCase(_template.Pluralize()), ToSnakeCase(_toName.Pluralize())) // StockCategories Samples
			.Replace(ToSnakeCase(_template), ToSnakeCase(_toName)) // StockCategories Samples
	...
	{
		return Regex.Replace(InitialLower(s), "[A-Z]", "_$0").ToLower();
	}
}
